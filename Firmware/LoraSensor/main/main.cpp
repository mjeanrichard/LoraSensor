...
void setupI2c()
{
    i2c_master_bus_config_t i2c_mst_config = {
        .i2c_port = I2C_NUM_0, .sda_io_num = PIN_SDA, .scl_io_num = PIN_SCL, .clk_source = I2C_CLK_SRC_DEFAULT, .glitch_ignore_cnt = 7, .intr_priority = 0, .trans_queue_depth = 0, .flags = 0};
...
extern "C" void app_main(void)
{
    ...
    _loraClient->sendMeasurements(temperature, humidity, adcMeasurements);

    if (wakeupSource == ESP_SLEEP_WAKEUP_GPIO || hasUsb)
    {
        _loraClient->startListening();
        int64_t time = esp_timer_get_time();
        while (esp_timer_get_time() - time < 10000000)
        {
            _loraClient->getData();
            vTaskDelay(1);
        }
